...
class AI(object):
    ...
    def classify(self, sensor_data):
        ...
        lista_Ap = pd.read_csv("Listado_ApsF.csv")
        ...
        #self.logger.debug(self.csv_dataClassify.shape)
        ...
        try:
            if name == "LSTM":              
                ...
                if (step == 20):
                    ...
                    model_new= load_model('E20p10milTmilEp.h5')
                    self.logger.debug("PASOS")
                    self.logger.debug(pasos)
                    prediction = model_new(pasos)
                else:
                    self.logger.debug("Dentro del else")
                    pasos1 = np.empty([20,self.csv_dataClassify.shape[1]])
                    self.logger.debug(pasos1.shape)
                    self.logger.debug(huella)
                    self.logger.debug(huella.shape)
                    for i in range(20):          
                        pasos1[i] = huella
                        #self.logger.debug(pasos1[i])
                    pasos1 = pasos1.reshape(1,20,self.csv_dataClassify.shape[1])
                    self.logger.debug(pasos1.shape)
                    model_new= load_model('E20p10milTmilEp.h5')
                    prediction = model_new(pasos1)
    ...
    def learn(self, fname):
        ...
        t = time.time()
        configs = json.load(open('config.json', 'r'))
        #Cargo en fname el archivo que contiene las huellas para entrenar
        fname = "../testing/HuellasPiso1.csv"
        #Ge
        #train3D_X, test3D_X, train3D_y, test3Y_y = ai.trayecto(fname)

        self.header = []
        rows = []
        naming_num = 0
        with open('datos_final.csv', 'r') as csvfile:          
        ...
        for name, clf in zip(names, classifiers):
            ...
            try:
                if name == "LSTM":
                    var = 0
                    #self.algorithms[name] = self.model.train(train3D_X, train3D_y,epochs =5,batch_size = 2,verbose=2,shuffle=True)
                    #self.model.save()
                    self.algorithms[name] = 'LSTM'
                    self.logger.debug(self.algorithms)
                else:
