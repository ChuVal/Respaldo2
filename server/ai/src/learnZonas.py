...
class AI(object):
    ...
    def classify(self, sensor_data):
        #self.logger.debug(sensor_data.shape)
        #header = self.header[1:]
        datos = pd.read_csv('datos_final.csv')
        header = list(datos.columns[0:50])
        is_unknown = True
        ...
        try:
            if name == "LSTM":
                ...
                for h in range(csv_dataClassify.shape[0]):
                    ...
                    huella = huella.reshape(50,1)
                    min_max_scaler = MinMaxScaler()
                    x_scaled = min_max_scaler.fit_transform(huella)
                    huella = x_scaled.reshape(1,50)
                    pasos[csv_dataClassify.shape[0]-1-h] = huella
                
                self.logger.debug(pasos.shape)
                if (step == 20):
                    pasos = pasos.reshape(1,step,50)
                    model_new= load_model('MatrizPMIMOM7.h5')
                    prediction = model_new(pasos)
                    self.logger.debug(prediction)
                else:
                    pasos2 = np.empty([20,csv_dataClassify.shape[1]])
                    for i in range(20):
                        pasos2[i] = huella
                    pasos2 = pasos2.reshape(1,20,csv_dataClassify.shape[1])
                    model_new= load_model('MatrizPMIMOM7.h5')
                    prediction = model_new(pasos2)
                    self.logger.debug(prediction)
